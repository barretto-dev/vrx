...
Stationkeeping::Stationkeeping() : Node("stationkeeping")
{
  ...
  deadband_m_ = this->declare_parameter<double>("deadband_m", 0.5);
  ...
  left_pos_ccw_  = this->declare_parameter<double>("left_pos_ccw",  +1.0);
  right_pos_ccw_ = this->declare_parameter<double>("right_pos_ccw", -1.0);
  ...
  brake_ticks_total_= this->declare_parameter<int>("brake_ticks", 8);

  // PID distância
  pid_dist_.kp = this->declare_parameter<double>("dist_kp", 20.0);
  pid_dist_.ki = this->declare_parameter<double>("dist_ki", 0.0);
  pid_dist_.kd = this->declare_parameter<double>("dist_kd", 0.0);
  pid_dist_.i_min = this->declare_parameter<double>("dist_i_min", -2.0);
  pid_dist_.i_max = this->declare_parameter<double>("dist_i_max",  2.0);
...
void Stationkeeping::control_tick()
{
  ...
  if (dist < slow_radius_m_) {
    ...
    double alpha = (dist - deadband_m_) / (slow_radius_m_ - deadband_m_); // 0..1
...
u_fwd = clamp(u_fwd, thrust_min_, thrust_cap);
  ...
  if (use_azimuth_spin_ && dist < 5.0 && std::fabs(yaw_err) > 15.0 * M_PI / 180.0) {
